...
public class JavaInterviewApplicationTests {
	...
	private boolean canWin(Integer leap, Integer[] game) {       
		int index = 0;
//		for(;index < game.length; index++) {
//			if(game[index] > 0 ) break;
//		}
...
//		Integer lastIndex = index;
        ...
        while(index < game.length) {
            ...
            if((index < 0) || (game[index] > 0) ) {
                return false;
            } else if(((index + leap) >= game.length) || (index+1 >= game.length )) {
                 return true;
            }            
//            game[lastIndex] = 1;  
            if( indexMap.get(index) > game.length/2) {
            	return false;
            }
            System.out.println("index: "+ index);
            System.out.println("game[index]: "+ game[index]);
            if(index + leap <= game.length)
            	System.out.println("game[index + leap]: "+ game[index + leap]);
            if(index + 1 <= game.length)
            System.out.println("game[index + 1]: "+ game[index + 1]);
            if(index - 1 >= 0)
            System.out.println("game[index - 1]: "+ game[index - 1]);
            if((leap != 0) && (game[index + leap] == 0)) {
                index = index + leap;
            } else if(game[index + 1].equals(0)) {
            	System.out.println("game[index + 1]: "+ game[index + 1]);
                index = index + 1;
            } else if(game[index - 1].equals(0)) {
            	System.out.println("game[index - 1]: "+ game[index - 1]);
               ...
               else if (index >= game.length) return true;
            } catch(Exception e) {
                if(index < 0) return false;
                else if (index >= game.length) return true;            	
	...
	void test68() {
        ...
        while (q-- > 0) {
            ...
            Integer leap = scan.nextInt();
            
            Integer[] game = new Integer[n];
            for (int i = 0; i < n; i++) {
                game[i] = scan.nextInt();
            }
            String s2 = (canWin(leap, game)) ? "YES" : "NO";
            try {
